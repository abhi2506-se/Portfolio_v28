...
export function ProjectDetailClient({ project, slug }: ProjectDetailClientProps) {
  ...
  const analyze = async (force = false) => {
    ...
    try {
      ...
      setError((e as Error).message || 'Failed to analyze project')
    } finally {
      setLoading(false)
    }
  }
  ...
  // Build tabs: Overview is always present; analysis tabs appear when analysis loads
  const analysisAvailable = !!analysis
  const allTabs = [
    { value: 'overview', icon: BarChart3,    label: 'Overview',   always: true  },
    { value: 'diagrams', icon: Layers,       label: 'Diagrams',   always: false },
    { value: 'github',   icon: Github,       label: 'Analytics',  always: false },
    { value: 'database', icon: Database,     label: 'Database',   always: false },
    { value: 'files',    icon: BookOpen,     label: 'Files',      always: false },
    { value: 'readme',   icon: Code2,        label: 'README',     always: false },
    { value: 'chat',     icon: MessageSquare,label: 'AI Chat',    always: false },
  ]
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview">
            ...
            {error && !loading && hasRepo && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-amber-700 dark:text-amber-400">GitHub analysis unavailable</p>
          ...
          {/* Analysis-powered tabs (only rendered when analysis exists) */}
          {analysis && (
            <>
